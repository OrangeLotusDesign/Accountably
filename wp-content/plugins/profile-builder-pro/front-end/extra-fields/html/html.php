...
function wppb_html_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ) {
    if ( $field['field'] == 'HTML' ) {
        ...
        if( $form_location != 'back_end' ) {
            $output = '
				<label for="'. $field['meta-name'] .'">'. $item_title .'</label>
				<span class="custom_field_html '. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" id="'. $field['meta-name'] .'" '. $extra_attr .'>'. do_shortcode( $field['html-content'] ) .'</span>';
            ...
            $output = '
				<table class="form-table">
					<tr>
						...
						<td>
							<span class="custom_field_html" id="'. $field['meta-name'] .'" '. $extra_attr .'>'. do_shortcode( $field['html-content'] ) .'</span>
