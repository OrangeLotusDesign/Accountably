	...
	foreach ( $userlisting_posts as $key => $value ){
		if ( trim( Wordpress_Creation_Kit_PB::wck_generate_slug( $value->post_title ) ) == $name ){
			...
			if( !empty( $userID ) ){
				...
				return '<div class="wppb-userlisting-container">'.(string) new PB_Mustache_Generate_Template( wppb_generate_mustache_array_for_user_list(), $userlisting_template, array( 'userlisting_form_id' => $value->ID, 'meta_key' => $meta_key, 'meta_value' => $meta_value, 'include' => $include, 'exclude' => $exclude ) ) . '</div>';
            }
		}
	}
}
        ...
        if( !empty( $userlisting_args[0] ) ){
			...
			if ( isset( $_REQUEST['searchFor'] ) ) {
                ...
                if (trim($search_for) !== $searchText){
                    ...
                    $args['search_columns'] = array('ID', 'user_login', 'user_email', 'user_url', 'display_name', 'user_nicename' );
                    ...
                    $user_meta_keys = array('first_name', 'last_name', 'nickname', 'description', 'wp_capabilities');
                    $wppb_manage_fields = get_option('wppb_manage_fields', 'not_found');
                    if ($wppb_manage_fields != 'not_found') {
...
function wppb_user_query_modifications($query) {
    ...
    if( !empty( $userlisting_args ) ){
        ...
        switch( $sorting_criteria ){
            case "role":
                $query->query_orderby = 'ORDER by REPLACE( wp_usermeta.meta_value, SUBSTRING_INDEX( wp_usermeta.meta_value, \'"\', 1 ), \'\' ) '.$sorting_order;
                break;
            case "RAND()":
        ...
        if ( isset( $_REQUEST['searchFor'] ) ) {
            ...
            $query->query_where = str_replace( "user_nicename LIKE '%".esc_attr($_REQUEST['searchFor'])."%')", "user_nicename LIKE '%".esc_attr($_REQUEST['searchFor'])."%'  OR user_registered LIKE '%". esc_attr($_REQUEST['searchFor']) ."%') ) )", $query->query_where );
        }
    }
}
...
function wppb_userlisting_faceted_menus( $value, $name, $children, $extra_info ){
    ...
    if( !empty( $this_faceted_filters ) ){
        ...
        foreach( $this_faceted_filters as $this_faceted_filter ){
            ...
            $meta_values = wppb_get_all_values_for_user_meta( $this_faceted_filter['facet-meta'], $this_faceted_filter['facet-behaviour'] );
...
function wppb_ul_faceted_checkboxes( $faceted_filter_options, $meta_values, $wppb_manage_fields ){
    ...
    if( !empty( $meta_values ) ){
        ...
        return __( 'No options available', 'profile-builder' );
}
...
function wppb_ul_faceted_range( $faceted_filter_options, $meta_values, $wppb_manage_fields ){
    ...
    if( $filter == '' )
        $filter = __( 'No options available', 'profile-builder' );
 ...
 * @return array
 */
function wppb_get_all_values_for_user_meta( $meta_key, $filter_behaviour ){
    $results = array();
    if( !empty( $meta_key ) ) {
        global $all_queried_user_ids_string;
...
function wppb_manage_ul_cpt(){
    ...
    $settings_fields = array(
        ...
        array( 'type' => 'select', 'slug' => 'facet-meta', 'title' => __( 'Facet Meta', 'profile-builder' ), 'options' => $facet_meta, 'description' => __( 'Choose the meta field for the facet menu', 'profile-builder' ) ),
    ...
    $settings_fields = array(
        array( 'type' => 'checkbox', 'slug' => 'search-fields', 'options' => $search_fields, 'default' => $search_defaults,  'title' => __( 'Search Fields', 'profile-builder' ), 'description' => __( 'Choose the fields in which the Search Field will look in', 'profile-builder' ) ),
    );
    // set up the box arguments
    $args = array(
